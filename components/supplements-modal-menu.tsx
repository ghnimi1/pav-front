...
import type { SelectedSupplement } from "@/contexts/breakfast-context"

type SharedSupplementConfig = {
  supplementId?: string
  isEnabled?: boolean
  customPrice?: number
  id?: string
...
type ItemWithSupplements = {
  ...
  description?: string
  image?: string
  availableSupplements?: SharedSupplementConfig[]
}

interface SupplementsModalProps {
  isOpen: boolean
  onClose: () => void
  item: ItemWithSupplements | null
  onConfirm: (item: ItemWithSupplements, supplements: SelectedSupplement[]) => void
}
...
export function SupplementsModal({ isOpen, onClose, item, onConfirm }: SupplementsModalProps) {
  ...
  const getAvailableSupplements = (): Supplement[] => {
    ...
    return item.availableSupplements
      .filter((ps) => ps.isEnabled !== false)
      .map(ps => {
        const supplementId = ps.supplementId || ps.id
        const supplement = allSupplements.find(
          (s) => s.id === supplementId && s.isActive !== false
        )

        const baseSupplement: Supplement | null = supplement
          ? supplement
          : ps.name
            ? {
                id: supplementId || `supplement-${ps.name}`,
                name: ps.name,
                price: ps.price ?? 0,
                description: ps.description,
                points: ps.points,
                category: ps.category,
                isActive: ps.isActive !== false && ps.isactive !== false,
                createdAt: new Date().toISOString(),
                updatedAt: new Date().toISOString(),
              }
            : null

        if (!baseSupplement) return null

        return ps.customPrice !== undefined
          ? { ...baseSupplement, price: ps.customPrice }
          : baseSupplement
      })
      .filter((supplement, index, supplements) => {
        return supplements.findIndex((current) => current?.id === supplement?.id) === index
      })
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto py-2">
          ...
          {availableSupplements.length === 0 && (
            <div className="rounded-lg border border-dashed border-stone-200 bg-stone-50 px-4 py-6 text-center text-sm text-stone-500">
