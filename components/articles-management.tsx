...
import { PlusIcon, PencilIcon, TrashIcon, SearchIcon, PackageIcon, FilterIcon, PackageSearchIcon } from "lucide-react"

interface ArticlesManagementProps {
  onNavigateToBatches?: (productId: string) => void
}

export function ArticlesManagement({ onNavigateToBatches }: ArticlesManagementProps) {
    ...
    return items
      .filter((item) => {
        ...
        if (statusFilter === "low") {
          ...
          matchesStatus = item.quantity > item.minQuantity && item.quantity <= item.minQuantity * 1.5
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-balance text-2xl font-semibold text-foreground">Gestion des Produits - Système FIFO</h2>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <Card className="p-4">
          <div className="space-y-1">
            ...
            <p className="text-2xl font-semibold text-foreground">{filteredItems.length}</p>
          </div>
        </Card>
        <Card className="p-4">
          <div className="space-y-1">
      ...
      <Card className="overflow-hidden">
          ...
          <table className="w-full">
            <thead className="bg-muted/50">
              <tr className="border-b border-border">
                ...
                <th className="p-4 text-left text-sm font-medium text-muted-foreground">Catégorie</th>
                ...
                <th className="p-4 text-left text-sm font-medium text-muted-foreground">Conserv. après ouv.</th>
                  ...
                  <tr key={item.id} className="hover:bg-muted/30">
                    ...
                    <td className="p-4">
                      <Badge variant="outline" className="gap-1">
                        ...
                        <span className="hidden sm:inline">{getCategoryName(item.category)}</span>
                      </Badge>
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="font-semibold text-foreground">{totalPrice.toFixed(2)} €</div>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      {item.shelfLifeAfterOpening ? (
                        <span className="text-sm text-foreground">{item.shelfLifeAfterOpening}j</span>
                      ) : (
        ...
        {filteredItems.length === 0 && (
          <div className="flex flex-col items-center justify-center p-12 text-center">
            ...
            <p className="mt-4 text-lg font-medium text-foreground">Aucun produit trouvé</p>
            <p className="text-sm text-muted-foreground">Essayez de modifier votre recherche ou vos filtres</p>
          </div>
        )}
