...
function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      addNotification({
        type: "error",
        title: "Le mot de passe doit contenir au moins 6 caracteres",
        category: "system",
      })
      return
    }
    ...
    if (success) {
      ...
      if (hasPendingCart) {
        addNotification({
          ...
          message: "Vous pouvez maintenant finaliser votre commande",
          category: "system",
        })
      }
      router.push("/menu")
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-teal-50 to-emerald-100 p-4">
      <Card className="w-full max-w-md p-8 shadow-xl">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600 mt-2">Créez votre compte pour commander</p>

          {hasPendingCart && (
            <div className="mt-4 rounded-lg bg-amber-50 border border-amber-200 p-3">
              <div className="flex items-center justify-center gap-2 text-amber-800">
                <ShoppingCartIcon className="h-4 w-4" />
                <p className="text-sm font-medium">Créez votre compte pour finaliser votre commande</p>
              </div>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <UserPlusIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-400" />
              <Input
                id="referralCode"
                type="text"
            ...
            <p className="text-xs text-gray-500">Vous avez un code de parrainage? Entrez-le pour recevoir des points bonus!</p>
          </div>
